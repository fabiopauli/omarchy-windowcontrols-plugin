...
The installer writes three executables under `~/.local/bin` and an explicitly marked block under `~/.config/hypr/bindings.lua`. It backs up an existing bindings file before replacement. Before overriding an unmanaged shortcut, it warns with the shortcut and matching binding line; an existing managed block is left unchanged. Runtime commands write only `~/.cache/hypr-minimized-stack`; the list command derives membership from Hyprland's actual `special:minimized` workspace and atomically prunes stale ordering records. Its `--dry-run` mode does not modify that file.
